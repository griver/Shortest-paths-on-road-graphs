...
namespace tnr {
	...
	public:
		full_tnr_path_finder(graph_t const & graph, shared_ptr<full_access_map>& access_nodes, shared_ptr<dist_table>& table, path_map* path)
	...
	private:
		...
		edge_weight find_dist(vertex_id start, vertex_id end, vertex_id &start_access, vertex_id &end_access) {
			unordered_map<vertex_id, full_access_info>::const_iterator start_iter = (*access_nodes)[start].begin();
			unordered_map<vertex_id, full_access_info>::const_iterator start_end = (*access_nodes)[start].end();
			unordered_map<vertex_id, full_access_info>::const_iterator end_iter = (*access_nodes)[end].begin();
			unordered_map<vertex_id, full_access_info>::const_iterator end_end = (*access_nodes)[end].end();
			float min_dist = DBL_MAX;
			float sum_dist;

			unordered_map<vertex_id, full_access_info>::const_iterator end_start = (*access_nodes)[end].begin();
