...
namespace tnr{
	...
	private:
		...
		grid_info const & info;
	public:
		shortcut_preprocessor(graph_t &old_graph, grid_info const & info): old_graph(old_graph), new_graph(new graph_t()), info(info) {
			mapping.resize(old_graph.v_count());
			r_mapping.resize(old_graph.v_count());
			excluded.resize(old_graph.v_count());
			excluded.assign(excluded.size(), false);
	...
	private:
		...
		size_t get_degree(vertex_id id) {
			size_t degree = 0;
